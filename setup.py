...
exec(open(join(dirname(__file__), 'actpy', 'release.py'), 'rb').read())  # Load release variables
lib_name = 'actpy'
...
def py2exe_datafiles():
    ...
    for root, dirnames, filenames in os.walk('actpy'):
...
def py2exe_options():
    if os.name == 'nt':
        ...
        return {
            'console': [
                {'script': 'actpy-bin', 'icon_resources': [
                    (1, join('setup', 'win32', 'static', 'pixmaps', 'actpy-icon.ico'))
                ]},
            ],
            'options': {
                'py2exe': {
