:banner: banners/actpy_external_api.jpg
:types: api
...
actpy is usually extended internally via modules, but many of its features and
all of its data are also available from the outside for external analysis or
integration with various tools. Part of the :ref:`reference/orm/model` API is
easily available over XML-RPC_ and accessible from a variety of languages.

.. actpy XML-RPC idiosyncracies:
   * uses multiple endpoint and a nested call syntax instead of a
     "hierarchical" server structure (e.g. ``actpy.res.partner.read()``)
...
.. only:: html
    ...
    .. rst-class:: setupcode hidden
        ...
        .. code-block:: python
            ...
            import xmlrpclib
            info = xmlrpclib.ServerProxy('https://demo.actpy.com/start').start()
        ...
        .. code-block:: ruby
            ...
            info = XMLRPC::Client.new2('https://demo.actpy.com/start').call('start')
            url, db, username, password = \
                info['host'], info['database'], info['user'], info['password']
        ...
        .. code-block:: php
            ...
            $info = ripcord::client('https://demo.actpy.com/start')->start();
        ...
        .. code-block:: java
            ...
            start_config.setServerURL(new URL("https://demo.actpy.com/start"));
...
If you already have an actpy server installed, you can just use its
parameters

.. warning::

    For actpy Online instances (<domain>.actpy.com), users are created without a
    *local* password (as a person you are logged in via the actpy Online
    authentication system, not by the instance itself). To use XML-RPC on actpy
    ...
    *https://mycompany.actpy.com*), the *database name* is the name of the
...
To make exploration simpler, you can also ask https://demo.actpy.com for a test
database:
...
actpy requires users of the API to be authenticated before they can query most 
data.
...
The second endpoint is ``xmlrpc/2/object``, is used to call methods of actpy
...
Records can be listed and filtered via :meth:`~actpy.models.Model.search`.

:meth:`~actpy.models.Model.search` takes a mandatory
...
:meth:`~actpy.models.Model.search_count` can be used to retrieve
only the number of records matching the query. It takes the same
:ref:`domain <reference/orm/domains>` filter as
:meth:`~actpy.models.Model.search` and no other parameter.
...
Record data is accessible via the :meth:`~actpy.models.Model.read` method,
which takes a list of ids (as returned by
:meth:`~actpy.models.Model.search`) and optionally a list of fields to
...
:meth:`~actpy.models.Model.fields_get` can be used to inspect
...
Because it is a very common task, actpy provides a
:meth:`~actpy.models.Model.search_read` shortcut which as its name suggests is
equivalent to a :meth:`~actpy.models.Model.search` followed by a
:meth:`~actpy.models.Model.read`, but avoids having to perform two requests
and keep ids around.

Its arguments are similar to :meth:`~actpy.models.Model.search`'s, but it
can also take a list of ``fields`` (like :meth:`~actpy.models.Model.read`,
...
Records of a model are created using :meth:`~actpy.models.Model.create`. The
method will create a single record and return its database identifier.

:meth:`~actpy.models.Model.create` takes a mapping of fields to values, used
...
.. warning::
    ...
    :class:`~actpy.fields.Integer`, string for :class:`~actpy.fields.Char`
    or :class:`~actpy.fields.Text`),

    * :class:`~actpy.fields.Date`, :class:`~actpy.fields.Datetime` and
      :class:`~actpy.fields.Binary` fields use string values
    * :class:`~actpy.fields.One2many` and :class:`~actpy.fields.Many2many`
      use a special command protocol detailed in :meth:`the documentation to
      the write method <actpy.models.Model.write>`.

Update records
--------------

Records can be updated using :meth:`~actpy.models.Model.write`, it takes
a list of records to update and a mapping of updated fields to values similar
to :meth:`~actpy.models.Model.create`.
...
:meth:`~actpy.models.Model.unlink`.
...
While we previously used :meth:`~actpy.models.Model.fields_get` to query a
model and have been using an arbitrary model from the start, actpy stores
...
Provides information about actpy models via its various fields
...
``field_id``
    list of the model's fields through a :class:`~actpy.fields.One2many` to
    :ref:`reference/webservice/inspection/fields`
``view_ids``
    :class:`~actpy.fields.One2many` to the :ref:`reference/views` defined
    for the model
``access_ids``
    :class:`~actpy.fields.One2many` relation to the
...
Provides information about the fields of actpy models and allows adding
custom fields without using Python code

``model_id``
    :class:`~actpy.fields.Many2one` to
...
``groups``
    ...
    :class:`~actpy.fields.Many2many` to ``res.groups``
