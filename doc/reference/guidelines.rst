:banner: banners/actpy_guidelines.jpg
...
This page introduces the new actpy Coding Guidelines. Those aim to improve the quality of the code (e.g. better readability of source) and actpy Apps. Indeed, proper code eases maintenance, aids debugging, lowers complexity and promotes reliability.
...
.. warning::
    ...
    each line. For more details, see our `pull request guide <https://actpy.com/submit-pr>`_.
...
Don't link data (image, libraries) outside actpy: do not use an
...
- The tag *<data>* is only used to set not-updatable data with ``noupdate=1``.
  ...
  set on the ``<actpy>`` tag and do not set a ``<data>`` tag.
...
actpy supports custom tags acting as syntactic sugar:
...
Using a linter can help show syntax and semantic warnings or errors. actpy
...
#. Imports of ``actpy``
#. Imports from actpy modules (rarely, and only if necessary)
...
.. code-block:: python
    ...
    # 2 :  imports of actpy
    import actpy
    from actpy import api, fields, models # alphabetically ordered
    from actpy.tools.safe_eval import safe_eval as eval
    from actpy.tools.translate import _
    # 3 :  imports from actpy modules
    from actpy.addons.website.models.website import slug
    from actpy.addons.web.controllers.main import login_redirect
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The actpy framework is in charge of providing the transactional context for
...
actpy uses a GetText-like method named "underscore" ``_( )`` to indicate that
...
In general in actpy, when manipulating strings, prefer ``%`` over ``.format()``
...
- Model name (using the dot notation, prefix by the module name) :
    - When defining an actpy Model : use singular form of the name (*res.partner*
      and *sale.order* instead of *res.partnerS* and *saleS.orderS*)
    - When defining an actpy Transient (wizard) : use ``<related_base_model>.<action>``
...
- actpy Python Class : use camelcase for code.
...
- Variable name :
    ...
    - actpy works with a record or a recordset, don't suffix variable names with
...
.. code-block:: javascript
    ...
    actpy.website.if_dom_contains('.jquery_class_selector', function () {
        /*your code here*/
    });
...
- **[CLA]** for signing the actpy Individual Contributor License
