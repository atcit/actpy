:banner: banners/actpy_javascript.jpg
...
* Global selectors should be avoided. Because a component may be used several
  times in a single page (an example in actpy is dashboards), queries should be
  ...
  lead to unintended or incorrect behavior.  actpy Web's
...
To display and interact with data, calls to the actpy server are necessary.
This is performed using :abbr:`RPC <Remote Procedure Call>`.

actpy Web provides two primary APIs to handle this: a low-level
JSON-RPC based API communicating with the Python section of actpy
Web (and of your module, if you have a Python part) and a high-level
API above that allowing your code to talk directly to high-level actpy models.
...
High-level API: calling into actpy models
-------------------------------------------

Access to actpy object methods (made available through XML-RPC from the server)
is done via :class:`Model`. It maps onto the actpy server objects via two primary
...
the actpy server object. Its usage is similar to that of the actpy Model API,
...
interface to searches (``search`` + ``read`` in actpy RPC terms). It
returns a :class:`~actpy.web.Query` object which is immutable but
allows building new :class:`~actpy.web.Query` instances from the
...
serialization methods, :func:`~actpy.web.Query.all` and
:func:`~actpy.web.Query.first`. These methods will perform a new
...
.. class:: Model(name)
    ...
    .. function:: Model.query(fields)
         ...
         :param Array<String> fields: list of fields to fetch during
                                      the search
         :returns: a :class:`~actpy.web.Query` object
                   representing the search to perform

.. class:: actpy.web.Query(fields)
    ...
    .. function:: actpy.web.Query.all()

        Fetches the result of the current :class:`~actpy.web.Query` object's
        search.

        :rtype: Deferred<Array<>>

    .. function:: actpy.web.Query.first()

       Fetches the **first** result of the current
       :class:`~actpy.web.Query`, or ``null`` if the current
       :class:`~actpy.web.Query` does have any result.

       :rtype: Deferred<Object | null>

    .. function:: actpy.web.Query.count()

       Fetches the number of records the current
       :class:`~actpy.web.Query` would retrieve.

       :rtype: Deferred<Number>

    .. function:: actpy.web.Query.group_by(grouping...)
       ...
       :param Array<String> grouping: Lists the levels of grouping
                                      ...
                                      varargs.
       :rtype: Deferred<Array<actpy.web.QueryGroup>> | null

    The second set of methods is the "mutator" methods, they create a
    **new** :class:`~actpy.web.Query` object with the relevant
    (internal) attribute either augmented or replaced.

    .. function:: actpy.web.Query.context(ctx)

       Adds the provided ``ctx`` to the query, on top of any existing
       context

    .. function:: actpy.web.Query.filter(domain)
    ...
    .. function:: actpy.web.Query.limit(limit)

       Sets the provided limit on the query. The new limit *replaces*
       the old one.

    .. function:: actpy.web.Query.order_by(fields…)
...
actpy has powerful grouping capacities, but they are kind-of strange
in that they're recursive, and level n+1 relies on data provided
directly by the grouping at level n. As a result, while
:py:meth:`actpy.models.Model.read_group` works it's not a very intuitive
API.

actpy Web eschews direct calls to :py:meth:`~actpy.models.Model.read_group`
in favor of calling a method of :class:`~actpy.web.Query`, :py:meth:`much
...
The result of a (successful) :func:`~actpy.web.Query.group_by` is
an array of :class:`~actpy.web.QueryGroup`:

.. class:: actpy.web.QueryGroup

    .. function:: actpy.web.QueryGroup.get(key)
    ...
    .. function:: actpy.web.QueryGroup.query([fields...])

        equivalent to :func:`Model.query` but pre-filtered to
        only include the records within this group. Returns a
        :class:`~actpy.web.Query` which can be further manipulated as
        usual.

    .. function:: actpy.web.QueryGroup.subgroups()

        returns a deferred to an array of :class:`~actpy.web.QueryGroup`
        below this one
...
It has many advantages over the actpy version 8 system.
...
* files are required to use the module system if they want to interact 
  with actpy, since the various objects are only available in the module
...
adopt new habits.  For example, the variable actpy does not exist
...
simple example::
    ...
    actpy.define('addon_name.service', function (require) {
...
with the ``actpy.define`` function.  ``actpy.define`` takes a name and a
...
* the ``framework/`` folder contains all basic low level modules.  The
  ...
  * ``web.session`` is the former ``actpy.session``
...
global variable ``actpy``.  In theory, our addons should work without
ever using the variable ``actpy``, and the compatibility module can be
disabled safely.
...
=============================
Testing in actpy Web Client
=============================

Javascript Unit Testing
-----------------------

actpy Web includes means to unit-test both the core code of
actpy Web and your own javascript modules. On the javascript side,
unit-testing is based on QUnit_ with a number of helpers and
extensions for better integration with actpy.

To see what the runner looks like, find (or start) an actpy server
...
``test`` key of the actpy manifest, by adding javascript files to it:
...
The next step is to create a test case::
    ...
    actpy.testing.section('basic section', function (test) {
        test('my first test', function () {
            ok(false, "this test has run");
        });
    });

All testing helpers and structures live in the ``actpy.testing``
module. actpy tests live in a :func:`~actpy.testing.section`,
...
:func:`test <actpy.testing.case>`, provided by the
:func:`~actpy.testing.section` to the callback, is used to
register a given test case which will be run whenever the test runner
actually does its job. actpy Web test case use standard `QUnit
...
As noted above, actpy Web's tests use `qunit assertions`_. They are
...
Getting an actpy instance
---------------------------

The actpy instance is the base through which most actpy Web
...
::
    ...
    // src/js/demo.js
    actpy.web_tests_demo = function (instance) {
...
e.g. fully initialize :class:`widgets <~actpy.Widget>` before
testing them.
...
.. function:: mock(rpc_spec, handler)
    ...
    * If it matches the pattern ``model:method`` (if it contains a
      ...
      call straight to the actpy server (through XMLRPC) as
      performed via e.g. :func:`actpy.web.Model.call`.
    ...
    * Otherwise, if it matches an absolute path (e.g. ``/a/b/c``) it
      ...
      as if returned by the original JSON-RPC handler::
          ...
          test('JSON-RPC', {rpc: 'mock', asserts: 3, templates: true}, function (instance, $s, mock) {
              ...
              instance.webclient = {toggle_bars: actpy.testing.noop};
...
.. function:: actpy.testing.section(name[, options], body)
    ...
    :param body:
    :type body: Function<:func:`~actpy.testing.case`, void>

.. function:: actpy.testing.case(name[, options], callback)
...
.. class:: TestOptions
    ...
    :func:`~actpy.testing.section` or
    :func:`~actpy.testing.case`. Except for
    :attr:`~TestOptions.setup` and
    :attr:`~TestOptions.teardown`, an option on
    :func:`~actpy.testing.case` will overwrite the corresponding
    option on :func:`~actpy.testing.section` so
    e.g. :attr:`~TestOptions.rpc` can be set for a
    :func:`~actpy.testing.section` and then differently set for
    some :func:`~actpy.testing.case` of that
    :func:`~actpy.testing.section`
...
#. Install a new database with all relevant modules (all modules with
   ...
   .. note::
       ...
       being duplicated, but actpy doesn't currently break
...
.. note::
    ...
    but all the web tests for all of actpy. This can be surprising.
