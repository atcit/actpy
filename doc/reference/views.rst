:banner: banners/actpy_views.jpg
...
``groups_id``
    :class:`~actpy.fields.Many2many` field to the groups allowed to view/use
    the current view
``inherit_id``
...
``button``
    ...
    ``type``
        type of button, indicates how it clicking it affects actpy:
    ...
    ``context``
        merged into the view's context when performing the button's actpy call
    ``confirm``
        confirmation message to display (and for the user to accept) before
        performing the button's actpy call
...
Semantic components tie into and allow interaction with the actpy
system. Available semantic components are:

``button``
  call into the actpy system, similar to :ref:`list view buttons
...
``field``
  ...
  ``widget``
    ...
    (e.g. :class:`~actpy.fields.Char`,
    :class:`~actpy.fields.Many2one`). The ``widget`` attributes allows using
  ...
  ``on_change``
    ...
    .. deprecated:: 8.0
       ...
       Use :func:`actpy.api.onchange` on the model
  ...
  ``mode``
    for :class:`~actpy.fields.One2many`, display mode (view type) to use for
  ...
  ``filename``
    ...
    file
  ``password``
    indicates that a :class:`~actpy.fields.Char` field stores a password and
...
.. image:: forms/actpy_account_line.png
   :class: img-responsive
...
Most :class:`~actpy.fields.Many2many` fields, like categories, are better
...
.. image:: forms/actpy_wizard_popup.png
   :class: img-responsive
...
``templates``
  ...
  ``record``
    ...
    a :meth:`~actpy.models.Model.read` (except for date and datetime fields
    that are `formatted according to user's locale
    <https://github.com/actpy/actpy/blob/a678bd4e/addons/web_kanban/static/src/js/kanban_record.js#L102>`_)
  ``read_only_mode``
    self-explanatory
...
``arrow`` (required, 1)
    ...
    ``object`` (required)
      the edge's actpy model
    ``source`` (required)
      :class:`~actpy.fields.Many2one` field of the edge's model pointing to
...
``field``
    ...
    ``widget``
        ...
        standard actpy 1.0 is a ``selection`` widget for
        :class:`~actpy.fields.Many2one` fields)
    ``domain``
        if the field can provide an auto-completion
        (e.g. :class:`~actpy.fields.Many2one`), filters the possible
        completion results.
...
``filter``
    ...
    ``domain``
        an actpy :ref:`domain <reference/orm/domains>`, will be appended to the
