:banner: banners/actpy_actions.jpg
...
``views``
    ...
    :meth:`~actpy.models.Model.fields_view_get`). The first type of the
...
Allow opening a URL (website/web page) via an actpy action. Can be customized
via two fields:
...
will replace the current content section by the actpy home page.
...
``model_id``
    actpy model linked to the action, made available in
...
Creates a new record, from scratch (via :meth:`~actpy.models.Model.create`)
or by copying an existing record (via :meth:`~actpy.models.Model.copy`)
...
``fields_lines``
    ...
    :class:`~actpy.fields.One2many` with the fields:
...
``crud_model_id``
    ...
    ``new_other``
``ref_object``
    :class:`~actpy.fields.Reference` to an arbitrary record to copy, used if
...
``groups_id``
    ...
    the current report
``paperformat_id``
    :class:`~actpy.fields.Many2one` field to the paper format you wish to
