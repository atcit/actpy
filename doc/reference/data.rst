:banner: banners/actpy_data_files.jpg
...
actpy is greatly data-driven, and a big part of modules definition is thus
...
The main way to define data in actpy is via XML data files: The broad structure
of an XML data file is the following:

* Any number of operation elements within the root element ``actpy``
...
``search``
    ...
    the field is a :class:`~actpy.fields.Many2one`
``ref``
    ...
    Mostly for :class:`~actpy.fields.Many2one` and
    :class:`~actpy.fields.Reference` fields
``type``
...
Because some important structural models of actpy are complex and involved,
...
.. literalinclude:: ../../actpy/addons/base/res/res.country.state.csv
    :language: text
    :lines: 1-15
...
.. csv-table::
    :file: ../../actpy/addons/base/res/res.country.state.csv
    :header-rows: 1
