:banner: banners/actpy_command_line_interface.jpg

.. _reference/cmdline:

===================================
Command-line interface: actpy-bin
===================================
...
.. option:: -s, --save
    ...
    (:file:`{$HOME}/.actpyrc` by default, and can be overridden using
    :option:`-c`)
...
.. option:: --db-filter <filter>
    ...
    - ``%d`` is replaced by the subdomain the request is made on, with the
      exception of ``www`` (so domain ``actpy.com`` and ``www.actpy.com`` both
      match the database ``actpy``).

      These operations are case sensitive. Add option ``(?i)`` to match all
      databases (so domain ``actpy.com`` using ``(?i)%d`` matches the database
      ``actpy``).
...
By default, actpy displays all logging of level_ ``info`` except for workflow
...
.. option:: --log-handler <handler-spec>
    ...
    e.g. ``actpy.models:DEBUG`` will enable all logging messages at or above
    ...
    * If the level is omitted, the logger is set to ``INFO``
    ...
    .. code-block:: console
        ...
        $ actpy-bin --log-handler :DEBUG --log-handler werkzeug:CRITICAL --log-handler actpy.fields:WARNING

.. option:: --log-request

    enable DEBUG logging for RPC requests, equivalent to
    ``--log-handler=actpy.http.rpc.request:DEBUG``

.. option:: --log-response

    enable DEBUG logging for RPC responses, equivalent to
    ``--log-handler=actpy.http.rpc.response:DEBUG``

.. option:: --log-web

    enables DEBUG logging of HTTP requests and responses, equivalent to
    ``--log-handler=actpy.http:DEBUG``

.. option:: --log-sql

    enables DEBUG logging of SQL querying, equivalent to
    ``--log-handler=actpy.sql_db:DEBUG``
...
.. option:: --log-level <level>
    ...
    ``actpy`` and ``werkzeug`` loggers (except for ``debug`` which is only
    set on ``actpy``).

    actpy also provides debugging pseudo-levels which apply to different sets
    of loggers:
    ...
    ``debug_rpc``
        sets the ``actpy`` and HTTP request loggers to ``debug``

        equivalent to ``--log-level debug --log-request``
    ``debug_rpc_answer``
        sets the ``actpy`` and HTTP request and response loggers to
        ``debug``
...
.. option:: --email-from <address>
    ...
    Email address used as <FROM> when actpy needs to send mails
...
.. option:: --smtp-ssl
    ...
    If set, actpy should use SSL/STARTSSL SMTP connections
...
.. program:: actpy-bin scaffold

Scaffolding is the automated creation of a skeleton structure to simplify
bootstrapping (of new modules, in the case of actpy). While not necessary it
avoids the tedium of setting up basic structures and looking up what all
starting requirements are.

Scaffolding is available via the :command:`actpy-bin scaffold` subcommand.
...
The default configuration file is :file:`{$HOME}/.actpyrc` which
can be overridden using :option:`--config <actpy-bin -c>`. Specifying
:option:`--save <actpy-bin -s>` will save the current configuration state back
to that file.
