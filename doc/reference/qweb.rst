...
QWeb is the primary templating_ engine used by actpy\ [#othertemplates]_. It
...
:meth:`actpy.http.HttpRequest.render`:
...
This automatically creates a :class:`~actpy.http.Response` object which can
...
.. py:method:: render(cr, uid, id[, values][, engine='ir.qweb][, context])
    ...
    ``request``
        the current :class:`~actpy.http.WebRequest` object, if any
    ``debug``
    ...
    :param str engine: name of the actpy model to use for rendering, can be
...
.. .. automodule:: actpy.addons.base.ir.ir_qweb
...
.. [#othertemplates] although it uses a few others, either for historical
                     ...
                     use case. actpy 9.0 still depends on Jinja_ and Mako_.
