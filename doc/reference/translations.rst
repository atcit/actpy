:banner: banners/actpy_translating_modules.jpg
...
automatically loaded by actpy when the corresponding language is installed (via
...
actpy automatically exports translatable strings from "data"-type content:
...
* QWeb templates (both server-side and client-side), all text nodes are
  ...
  exported
* for :class:`~actpy.fields.Field`, unless their model is marked with
  ``_translate = False``:
...
* help/error messages of :attr:`~actpy.models.Model._constraints` and
  :attr:`~actpy.models.Model._sql_constraints` are exported

Explicit exports
================

When it comes to more "imperative" situations in Python code or Javascript
code, actpy cannot automatically export translatable terms so they
must be marked explicitly for export. This is done by wrapping a literal
string in a function call.

In Python, the wrapping function is :func:`actpy._`::

    title = _("Bank Accounts")

In JavaScript, the wrapping function is generally :js:func:`actpy.web._t`:
