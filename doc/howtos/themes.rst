:banner: banners/actpy_theme_tutorial.jpg
...
actpy celebrates freedom. Freedom for the designer to go further and
freedom for the user to customize everything according to their needs.

Ready to create your own theme? Great. Here are some things you should know before you begin. This tutorial is a guide to creating an actpy theme.

.. image:: theme_tutorial_assets/img/actpy_intro.jpg


An introduction for web designers
=================================

If you are a web designer using actpy for the first time, you are in the right place.
This introduction will outline the basics of actpy theme creation.

.. note::

   actpy’s team has created a framework that’s powerful and easy to use. There’s no need to know special syntaxes to use this set of tools.

From common CMS to actpy
--------------------------
...
This is usually the first question from a web designer used to working with Wordpress or Joomla and coming to actpy for the first time.

.. container:: row

  .. container:: col-sm-5

     .. image:: theme_tutorial_assets/img/actpy_cms.jpg
  ...
  .. container:: col-sm-7
    ...
    This **does not** apply to creating actpy themes.
...
.. container:: row
  ...
  .. container:: col-sm-7
    ...
    actpy comes with a default theme structure.
    ...
    It will upgrade automatically within your actpy installation and, since it is included in the Website Builder module, everything is smoothly integrated by default.
  ...
  .. container:: col-sm-5
     ...
     .. image:: theme_tutorial_assets/img/actpy_structure.jpg
...
An actpy theme is not a folder containing HTML or PHP files, it’s a modular framework written in XML. Never worked with XML files before? Don’t worry, after following the tutorial, you’ll be able to create your first theme with only basic knowledge of HTML.
...
Creating themes for actpy is a total change of perspective. Instead of defining the complete layout for a page, you can create blocks (snippets) at let the user choose where to “drag&drop” them, creating the page layout on their own.
We call this modular design.

Imagine an actpy theme as a “list” of elements and options that you have to create and style.
...
.. container:: row
    ...
    .. container:: col-sm-6
        ...
        .. image:: theme_tutorial_assets/img/actpy_snippet.jpg
...
.. container:: row
    ...
    .. container:: col-sm-6
      ...
      .. image:: theme_tutorial_assets/img/actpy_page.jpg

.. container:: row

  .. figure:: theme_tutorial_assets/img/actpy_styles.jpg
     :figclass: col-sm-6
  ...
  .. figure:: theme_tutorial_assets/img/actpy_functionalities.jpg
     :figclass: col-sm-6

     Functionalities

     Thanks to actpy’s modularity, everything can be personalized even more. This means there are endless possibilities for your creativity. Adding functionalities is easy and it’s simple to provide the end user with customizable options.


actpy's XML files, an overview
--------------------------------

Any actpy XML file starts with encoding specifications.
After that, you have to write your code inside a ``<actpy>`` tag.
...
the **actpy default structure**.  In order to do that you can use
...
.. container:: row
  ...
  .. container:: col-sm-5
    ...
    .. image:: theme_tutorial_assets/img/actpy_upgrade_module.png
...
actpy’s themes are packaged like modules. Even if you are designing a very simple website for your company or client, you need to package the theme like an actpy module.
...
.. important::
  ...
  and two at the end of actpy and init file names.
...
These values will be used to identify your new theme in actpy’s backend.

The ``data`` property will contain the xml files list. Right now it’s empty, but we will add any new files created.

``category`` defines your module category (always “Theme”) and, after a slash, the subcategory.You can use one subcategory from the actpy Apps categories list. (https://store.actpy.com/apps/category/2)
...
To install your theme, you just place your theme folder inside addons in your actpy installation.

After that, navigate to the Settings page, look for your theme and click on the install button.

Structure of an actpy page
============================

An actpy page is the visual result of a combination of 2 kind of elements, **cross-pages** and **unique**.
By default, actpy provides you with a **Header** and a **Footer** (cross-pages) and a unique main element that contains the content that makes your page unique.

.. note::

  Cross-pages elements will be the same on every page. Unique elements are related to a specific page only.

.. image:: theme_tutorial_assets/img/actpy_page_structure.jpg
...
By default, actpy header contains a responsive navigation menu and the company’s logo. You can easily add new elements or style the existing one.

To do so, create a **layout.xml** file in your **views** folder and add the default actpy xml markup.
...
Create a new template into the ``<actpy>`` tag, copy-pasting the following
code.
...
.. warning::
  ...
  your changes will take priority in any future actpy’s update.
...
.. image:: theme_tutorial_assets/img/actpy_after_menu.png
   :class: shadow-0
...
default actpy markup.  Inside ``<actpy>`` create a ``<template>`` tag, set the
...
Using ``<t t-call="website.layout">`` we will extend the actpy
...
.. image:: theme_tutorial_assets/img/actpy_services_page_nostyle.png.png
   :class: shadow-0
...
.. image:: theme_tutorial_assets/img/actpy_services_page_menu.png
   :class: shadow-0
...
actpy includes Bootstrap by default. This means that you can take advantage of all Bootstrap styles and layout functionalities out of the box.
...
Let’s navigate to the view folder and create an XML file called *assets.xml*. Add the default actpy xml markup and copy/paste the following code. Remember to replace ``theme folder`` with your theme’s main folder name.
...
attribute tells actpy that our template is referring to another one in
order to operate.
...
Our less file is now included in our theme, it will be automatically compiled, minified and combined with all actpy’s assets.

.. image:: theme_tutorial_assets/img/actpy_services_page_styled.png
   :class: shadow-0
...
Add the default actpy xml markup and copy/paste the following code.
...
We start by adding a new file in our views folder - name it **options.xml** and add the default actpy XML markup. Create a new template copy/pasting the following
...
.. code-block:: javascript
    ...
    (function() {
        'use strict';
        var website = actpy.website;
        website.actpy_website = {};
    })();
...
.. image:: theme_tutorial_assets/img/actpy_snippet_custom_method.png
   :class: shadow-0
