:banner: banners/actpy_building_website.jpg
...
.. warning::
    ...
    * This guide assumes :ref:`an installed actpy <setup/install>`

Creating a basic module
=======================

In actpy, tasks are performed by creating modules.

Modules customize the behavior of an actpy installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`actpy's scaffolding <reference/cmdline/scaffold>` can setup a basic
module. To quickly get started simply invoke:

.. code-block:: console

    $ ./actpy-bin scaffold Academy my-modules
...
* start the actpy server

  .. code-block:: console

      $ ./actpy-bin --addons-path addons,my-modules
...
display logic. actpy allows any Python templating system, but provides its
...
Finally restart actpy and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, actpy can be restarted :option:`and update modules at
    the same time<actpy-bin -u>`:

    .. code-block:: console

        $ actpy-bin --addons-path addons,my-modules -d academy -u academy

Going to http://localhost:7073/academy/academy/ should now result in:

.. image:: website/actpy_basic-list.png

Storing data in actpy
=======================

:ref:`actpy models <reference/orm/model>` map to database tables.
...
#. Because :meth:`~actpy.models.Model.search` returns a set of records
...
actpy bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but actpy 8 added deeper
...
.. image:: website/actpy_layout.png
...
You're now in actpy "proper": the administrative interface. For now click on
...
:func:`~actpy.http.route` decorator which takes a routing string and a
...
restart actpy, access http://localhost:7073/academy/Alice/ and
...
Restart actpy, access http://localhost:7073/academy/2, note how the old value
...
actpy provides an additional converter called ``model`` which provides records
...
Restart actpy and upgrade the module, then you can visit each teacher's page.
...
Restart actpy and update the views, reload the teacher's page and… the field
...
Restart actpy and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the actpy administration
-----------------------------------------------------------------

The actpy administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the actpy backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which actpy should
   execute or data display. Data display actions are called *window actions*,
   and tell actpy to display a given *model* according to a set of views…
...
Editing in the actpy administration
-------------------------------------

By default, an actpy model is essentially invisible to a user. To make it
...
:ref:`view <reference/views>`) actpy will automatically create a basic one
...
:class:`the inverse relationship <actpy.fields.One2many>` to the *teachers*
model:
...
actpy provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of actpy's email and messaging
system, which can add notifications and discussion threads to any model.
The model simply has to :attr:`~actpy.models.Model._inherit`
...
actpy also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the actpy system. We can easily make
...
restart actpy, update your module, there is now a :guilabel:`Shop` section in
...
.. note::
    ...
    * to extend a model in-place, it's :attr:`inherited
      <actpy.models.Model._inherit>` without giving it a new
      :attr:`~actpy.models.Model._name`
