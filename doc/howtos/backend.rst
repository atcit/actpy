:banner: banners/actpy_building_module.jpg
...
.. warning::
    ...
    This tutorial requires :ref:`having installed actpy <setup/install>`

Start/Stop the actpy server
=============================

actpy uses a client/server architecture in which clients are web browsers
accessing the actpy server via RPC.
...
In order to start the server, simply invoke the command :ref:`actpy-bin
...
Build an actpy module
=======================

Both server and client extensions are packaged as *modules* which are
optionally loaded in a *database*.

actpy modules can either add brand new business logic to an actpy system, or
alter and extend existing business logic: a module can be created to add your
country's accounting rules to actpy's generic accounting support, while the
next module adds support for real-time visualisation of a bus fleet.

Everything in actpy thus starts and ends with modules.

Composition of a module
-----------------------

An actpy module can contain a number of elements:

Business objects
    Declared as Python classes, these resources are automatically persisted
    by actpy based on their configuration
...
are specified by using the :option:`--addons-path <actpy-bin --addons-path>`
option.
...
An actpy module is declared by its :ref:`manifest <reference/module/manifest>`.
...
actpy provides a mechanism to help set up a new module, :ref:`actpy-bin
<reference/cmdline/server>` has a subcommand :ref:`scaffold
<reference/cmdline/scaffold>` to create an empty module:

.. code-block:: console

    $ actpy-bin scaffold <module name> <where to put it>
...
.. exercise:: Module creation
    ...
    .. only:: solutions
        ...
        #. Invoke the command ``actpy-bin scaffold openacademy addons``.
...
A key component of actpy is the :abbr:`ORM (Object-Relational Mapping)` layer.
...
:class:`~actpy.models.Model` which integrates them into the automated
persistence system.

Models can be configured by setting a number of attributes at their
definition. The most important attribute is
:attr:`~actpy.models.Model._name` which is required and defines the name for
the model in the actpy system. Here is a minimally complete definition of a
model::
...
:attr:`~actpy.fields.Field.string` (``unicode``, default: field's name)
    The label of the field in UI (visible by users).
:attr:`~actpy.fields.Field.required` (``bool``, default: ``False``)
    If ``True``, the field can not be empty, it must either have a default
    value or always be given a value when creating a record.
:attr:`~actpy.fields.Field.help` (``unicode``, default: ``''``)
    Long-form, provides a help tooltip to users in the UI.
:attr:`~actpy.fields.Field.index` (``bool``, default: ``False``)
    Requests that actpy create a `database index`_ on the column.
...
Example of simple fields are :class:`~actpy.fields.Boolean`,
:class:`~actpy.fields.Date`, :class:`~actpy.fields.Char`.

Reserved fields
###############

actpy creates a few fields in all models\ [#autofields]_. These fields are
managed by the system and shouldn't be written to. They can be read if
useful or necessary:

:attr:`~actpy.fields.Model.id` (:class:`~actpy.fields.Id`)
    The unique identifier for a record in its model.
:attr:`~actpy.fields.Model.create_date` (:class:`~actpy.fields.Datetime`)
    Creation date of the record.
:attr:`~actpy.fields.Model.create_uid` (:class:`~actpy.fields.Many2one`)
    User who created the record.
:attr:`~actpy.fields.Model.write_date` (:class:`~actpy.fields.Datetime`)
    Last modification date of the record.
:attr:`~actpy.fields.Model.write_uid` (:class:`~actpy.fields.Many2one`)
    user who last modified the record.

Special fields
##############

By default, actpy also requires a ``name`` field on all models for various
display and search behaviors. The field used for these purposes can be
overridden by setting :attr:`~actpy.models.Model._rec_name`.
...
actpy is a highly data driven system. Although behavior is customized using
Python_ code part of a module's value is in the data it sets up when loaded.

.. tip:: some modules exist solely to add data into actpy
    :class: aphorism
...
* ``model`` is the name of the actpy model for the record.
...
If no search view exists for the model, actpy generates one which only allows
...
:class:`Many2one(other_model, ondelete='set null') <actpy.fields.Many2one>`
...
:class:`One2many(other_model, related_field) <actpy.fields.One2many>`
    A virtual relationship, inverse of a :class:`~actpy.fields.Many2one`.
    A :class:`~actpy.fields.One2many` behaves as a container of records,
    ...
    .. danger::
        ...
        Because a :class:`~actpy.fields.One2many` is a virtual relationship,
        there *must* be a :class:`~actpy.fields.Many2one` field in the
        :samp:`{other_model}`, and its name *must* be :samp:`{related_field}`

:class:`Many2many(other_model) <actpy.fields.Many2many>`
...
actpy provides two *inheritance* mechanisms to extend an existing model in a
modular way.
...
.. seealso::
    ...
    * :attr:`~actpy.models.Model._inherit`
    * :attr:`~actpy.models.Model._inherits`

View inheritance
----------------

Instead of modifying existing views in place (by overwriting them), actpy
...
In actpy, :ref:`reference/orm/domains` are values that encode conditions on
...
:attr:`~actpy.fields.Field.compute` to the name of a method. The computation
...
.. code-block:: python
    ...
    import random
    from actpy import models, fields, api
...
on the compute method with the decorator :func:`~actpy.api.depends`.
    ...
    * Add a field ``active`` in the class Session, and set sessions as active by
    ...
    .. only:: solutions
        ...
        .. note::
            ...
            actpy has built-in rules making fields with an ``active`` field set
            to ``False`` invisible.
...
the record in the form view, and decorate it with :func:`~actpy.api.onchange`
...
actpy provides two ways to set up automatically verified invariants:
:func:`Python constraints <actpy.api.constrains>` and
:attr:`SQL constraints <actpy.models.Model._sql_constraints>`.

A Python constraint is defined as a method decorated with
:func:`~actpy.api.constrains`, and invoked on a recordset. The decorator
...
:attr:`~actpy.models.Model._sql_constraints`. The latter is assigned to a list
...
.. exercise:: Add access control through the actpy interface
...
the same convention as the method :meth:`~actpy.models.Model.write` of the ORM.
...
:class:`~actpy.models.TransientModel` instead of
:class:`~actpy.models.Model`. The class
:class:`~actpy.models.TransientModel` extends :class:`~actpy.models.Model`
...
pt_BR.po). Translations will be loaded automatically by actpy for all
enabled languages. Developers always use English when creating a module, then
export the module terms using actpy's gettext POT export feature
...
.. tip:: The Portable Object files generated by actpy are published on
         `Transifex <https://www.transifex.com/actpy/public/>`__, making it
...
.. code-block:: text
   ...
   |- idea/ # The module directory
      |- i18n/ # Translation files
         | - idea.pot # Translation Template (exported from actpy)
...
.. tip:: 
   ...
   By default actpy's POT export only extracts labels inside XML files or
   inside field definitions in Python code, but any Python string can be
   translated this way by surrounding it with the function :func:`actpy._`
   (e.g. ``_("Label")``)

.. exercise:: Translate a module

   Choose a second language for your actpy installation. Translate your
   module using the facilities provided by actpy.
   ...
   .. only:: solutions
        ...
        #. In ``models.py``, add an import statement for the function
           ``actpy._`` and mark missing strings as translatable
...
actpy 11.0 uses a report engine based on :ref:`reference/qweb`,
...
actpy is accessible through XML-RPC/JSON-RPC interfaces, for which libraries
exist in many languages.

XML-RPC Library
---------------

The following example is a Python 3 program that interacts with an actpy
...
.. exercise:: Add a new service to the client
   ...
   actpy (yours, or your instructor's). This program should display all
...
The following example is a Python 3 program that interacts with an actpy server
...
.. note::
    ...
    There are a number of high-level APIs in various languages to access actpy
