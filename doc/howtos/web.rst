:banner: banners/actpy_building_interface_extensions.jpg
...
This guide is about creating modules for actpy's web client.

To create websites with actpy, see :doc:`website`; to add business capabilities
or extend existing business systems of actpy, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed actpy <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple actpy module holding basic web component
...
.. code-block:: console
    ...
    $ git clone http://github.com/actpy/petstore

This will create a ``petstore`` folder wherever you executed the command.
You then need to add that folder to actpy's
:option:`addons path <actpy-bin --addons-path>`, create a new database and
...
Files used in the "web" side of an actpy module must be placed in a ``static``
...
``oepetstore/static/js/petstore.js``
    ...
    currently look like::
        ...
        actpy.oepetstore = function(instance, local) {
...
.. warning::
    ...
    "developer mode": log into your actpy instance (user *admin* password
...
The actpy framework uses one such pattern to define modules within web addons,
...
In actpy web, modules are declared as functions set on the global ``actpy``
...
* the first parameter is the current instance of the actpy web client, it gives
  access to various capabilities defined by the actpy (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the actpy web client needs to call them
...
For simplicity and developer-friendliness actpy web provides a class
system based on John Resig's `Simple JavaScript Inheritance`_.

New classes are defined by calling the :func:`~actpy.web.Class.extend`
method of :class:`actpy.web.Class`::
...
The :func:`~actpy.web.Class.extend` method takes a dictionary describing
...
by calling :func:`~actpy.web.Class.extend` on the parent class, as is done
to subclass :class:`~actpy.web.Class`::
...
The actpy web client bundles jQuery_ for easy DOM manipulation. It is useful
...
actpy Web makes specific components responsible for sections of a page. In
actpy web, the base for such components is the :class:`~actpy.Widget`
...
It extends :class:`~actpy.Widget` and overrides the standard method
:func:`~actpy.Widget.start`, which — much like the previous ``MyClass``
— does little for now.
...
The ``HomePage`` widget already has a :func:`~actpy.Widget.start`
...
All widgets have a :attr:`~actpy.Widget.$el` which represents the
section of page they're in charge of (as a jQuery_ object). Widget content
should be inserted there. By default, :attr:`~actpy.Widget.$el` is an
empty ``<div>`` element.
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear actpy user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in actpy Web, you will need to reload
    the page. There is no need to restart the actpy server.

The ``HomePage`` widget is used by actpy Web and managed automatically.
...
``GreetingsWidget``'s :func:`~actpy.Widget.appendTo` method::
...
* Finally it tells ``GreetingsWidget`` where to insert itself, delegating part
  of its :attr:`~actpy.Widget.$el` to the ``GreetingsWidget``.

When the :func:`~actpy.Widget.appendTo` method is called, it asks the
widget to insert itself at the specified position and to display its content.
The :func:`~actpy.Widget.start` method will be called during the call
to :func:`~actpy.Widget.appendTo`.
...
:class:`~actpy.Widget`, because we added some classes on them.
...
When overriding the :func:`~actpy.Widget.init` method of a widget it is
...
it. This is done via the :func:`~actpy.Widget.destroy` method::

    greeting.destroy();

When a widget is destroyed it will first call
:func:`~actpy.Widget.destroy` on all its children. Then it erases itself
from the DOM. If you have set up permanent structures in
:func:`~actpy.Widget.init` or :func:`~actpy.Widget.start` which
must be explicitly cleaned up (because the garbage collector will not handle
them), you can override :func:`~actpy.Widget.destroy`.

.. danger::

    when overriding :func:`~actpy.Widget.destroy`, ``_super()``
...
As many other environments, actpy's solution is to use a `template engine`_.
actpy's template engine is called :ref:`reference/qweb`.
...
* It has special support in actpy Web's :class:`~actpy.Widget`, though it
  can be used outside of actpy's web client (and it's possible to use
  :class:`~actpy.Widget` without relying on QWeb)
...
.. note::
    ...
    like actpy :ref:`views <reference/views>`.
    ...
    characteristic of actpy, and losing it was considered unacceptable.
...
However, because :class:`~actpy.Widget` has special integration for QWeb
the template can be set directly on the widget via its
:attr:`~actpy.Widget.template` attribute::
...
* with the second version, the template is rendered right before
  :func:`~actpy.Widget.start` is called
...
.. warning::
    ...
    they're set as a widget's :attr:`~actpy.Widget.template`. If there are
...
When using :class:`~actpy.Widget`'s integration it is not possible to
...
before :func:`~actpy.Widget.start` is called (the widget's state will
essentially be that set up by :func:`~actpy.Widget.init`):
...
But because it's a common operation, :class:`~actpy.Widget` provides an
equivalent shortcut through the :func:`~actpy.Widget.$` method::
...
:attr:`~actpy.Widget.events` is an object (mapping) of an event to the
...
:func:`~actpy.Widget.trigger` takes the name of the event to trigger as
...
listening to the ``user_chose`` event using :func:`~actpy.Widget.on`::
...
:func:`~actpy.Widget.on` binds a function to be called when the
...
additional arguments of :func:`~actpy.Widget.trigger` if it has
any. Example::
...
.. note::
    ...
    exception to that rule is ``actpy.web.bus`` which exists specifically
    to broadcasts evens in which any widget could be interested throughout
    the actpy web application.
...
* :func:`~actpy.Widget.set` sets the value of a property and triggers
  :samp:`change:{propname}` (where *propname* is the property name passed as
  first parameter to :func:`~actpy.Widget.set`) and ``change``
* :func:`~actpy.Widget.get` retrieves the value of a property.
...
The class system of the actpy web framework allows direct modification of
existing classes using the :func:`~actpy.web.Class.include` method::
...
defined in the call to :func:`~actpy.web.Class.include`. This will also work
if some instances of the class (or of any of its sub-classes) were created
prior to the call to :func:`~actpy.Widget.include`.
...
In actpy, translations files are automatically generated by scanning the source
...
:func:`~actpy.web._t` (and also :func:`~actpy.web._lt`).
...
:func:`~actpy.web._lt` ("lazy translate") is similar but somewhat more
...
Communication with the actpy Server
=====================================

Contacting Models
-----------------

Most operations with actpy involve communicating with *models* implementing
...
communicating with actpy requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, actpy web provides
...
The class used to call actpy models is :class:`actpy.Model`. It is
instantiated with the actpy model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~actpy.web.Model.call` can be used to call any (public) method of an
actpy model. It takes the following positional arguments:
...
:func:`~actpy.Widget.call` returns a deferred resolved with the value
...
the time zone of the user, used to compute correctly dates and times if actpy
...
solution to achieve that is to use :class:`actpy.web.CompoundContext`.

:class:`~actpy.web.CompoundContext` is a class used to pass the user's
...
dictionaries or other :class:`~actpy.web.CompoundContext` instances. It will
...
are related to the configuration of the current user in actpy plus the
``new_key`` key that was added when instantiating
:class:`~actpy.web.CompoundContext`.

Queries
-------

While :func:`~actpy.Model.call` is sufficient for any interaction with actpy
models, actpy Web provides a helper for simpler and clearer querying of models
(fetching of records based on various conditions):
:func:`~actpy.Model.query` which acts as a shortcut for the common
combination of :py:meth:`~actpy.models.Model.search` and
::py:meth:`~actpy.models.Model.read`. It provides a clearer syntax to search
and read models::
...
* :func:`~actpy.web.Model.query` takes an optional list of fields as
  parameter (if no field is provided, all fields of the model are fetched). It
  returns a :class:`actpy.web.Query` which can be further customized before
  being executed
* :class:`~actpy.web.Query` represents the query being built. It is
  ...
  :class:`~actpy.web.Query` for its customization options.

When the query is set up as desired, simply call
:func:`~actpy.web.Query.all` to execute it and return a
deferred to its result. The result is the same as
:py:meth:`~actpy.models.Model.read`'s, an array of dictionaries where each
...
.. exercise:: Pet Toys List
    ...
    In actpy, images are generally stored in regular fields encoded as
...
In actpy, many operations start from an :ref:`action <reference/actions>`:
...
In actpy Web, the component responsible for handling and reacting to these
...
:func:`~actpy.Widget.do_action` is a shortcut of :class:`~actpy.Widget`
...
the actpy web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for actpy web. The server simply sends
...
``instance.web.client_actions`` is a :class:`~actpy.web.Registry` in which
the action manager looks up client action handlers when it needs to execute
one. The first parameter of :class:`~actpy.web.Registry.add` is the name
(tag) of the client action, and the second parameter is the path to the widget
from the actpy web client root.
...
Much of actpy web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`actpy views <reference/views>` are implemented through a subclass
of :class:`actpy.web.View` which provides a bit of generic basic structure
for handling events and displaying model information.

The *search view* is considered a view type by the main actpy framework, but
...
:py:class:`~actpy.models.Model.fields_view_get`) and any other data source
it needs. To that purpose, views are provided with an optional view
identifier set as the :attr:`~actpy.web.View.view_id` attribute.

Views are also provided with a :class:`~actpy.web.DataSet` instance which
...
:func:`~actpy.web.View.do_search`, and updating their
:class:`~actpy.web.DataSet` as necessary.
...
:py:class:`~actpy.fields.Char` fields which should contain email addresses
...
* It must correctly implement the 3 field attributes available in all fields
  of actpy. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in actpy have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the actpy server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in actpy. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the actpy web
client.
...
user to modify it can be useful, but most fields in actpy also allow editing.
...
their :attr:`~actpy.web.form.FormWidget.field_manager` attribute::
...
:attr:`~actpy.web.form.FormWidget` is generally the
:class:`~actpy.web.form.FormView` itself, but features used from it should
be limited to those defined by :class:`~actpy.web.form.FieldManagerMixin`,
the most useful being:

* :func:`~actpy.web.form.FieldManagerMixin.get_field_value(field_name)`
  which returns the value of a field.
* :func:`~actpy.web.form.FieldManagerMixin.set_values(values)` sets multiple
