:banner: banners/actpy_deploying.jpg

=================
Deploying actpy
=================

This document describes basic steps to set up actpy in production or on an
...
actpy is a multi-tenant system: a single actpy system may run and serve a number
...
bound to a database: actpy needs to know which database should be used to load
...
accessible actpy needs a rule to know which one it should use.

That is one of the purposes of :option:`--db-filter <actpy-bin --db-filter>`:
...
.. note::
  Setting a proper :option:`--db-filter <actpy-bin --db-filter>` is an important part
...
UNIX socket is fine if you want actpy and PostgreSQL to execute on the same
machine, and is the default when no host is provided, but if you want actpy and
...
* Only accept loopback connections and `use an SSH tunnel`_ between the
  machine on which actpy runs and the one on which PostgreSQL runs, then
  configure actpy to connect to its end of the tunnel
* Accept connections to the machine on which actpy is installed, possibly
  over ssl (see `PostgreSQL connection settings`_ for details), then configure
  actpy to connect over the network
...
Out of the box, actpy connects to a local postgres over UNIX socket via port
...
create a new user (``actpy``) and set it as the database user.
...
actpy includes built-in HTTP servers, using either multithreading or
multiprocessing.
...
* Multiprocessing is enabled by configuring :option:`a non-zero number of
  worker processes <actpy-bin --workers>`, the number of workers should be based
...
listening on :option:`the longpolling port <actpy-bin --longpolling-port>` but
the client will not connect to it.

Instead you must have a proxy redirecting requests whose URL starts with
``/longpolling/`` to the longpolling port. Other request should be proxied to
the :option:`normal HTTP port <actpy-bin --http-port>`
...
* RAM = 9 * ((0.8*150) + (0.2*1024)) ~= 3Go RAM for actpy
...
Whether it's accessed via website/web client or web service, actpy transmits
authentication information in cleartext. This means a secure deployment of
actpy must use HTTPS\ [#switching]_. SSL termination can be implemented via
just about any SSL termination proxy, but requires the following setup:

* Enable actpy's :option:`proxy mode <actpy-bin --proxy-mode>`. This should only be enabled when actpy is behind a reverse proxy
...
in ``/etc/nginx/sites-enabled/actpy.conf`` set:
...
.. code-block:: nginx
  ...
  server {
     listen 80;
     server_name actpy.mycompany.com;
  ...
  server {
   ...
   # Add Headers for actpy proxy mode
   ...
   # log
   access_log /var/log/nginx/actpy.access.log;
   error_log /var/log/nginx/actpy.error.log;
   
   # Redirect requests to actpy backend server
   location / {
     proxy_redirect off;
     proxy_pass http://actpy;
   }
   location /longpolling {
       proxy_pass http://actpychat;
   }
...
actpy as a WSGI Application
=============================

It is also possible to mount actpy as a standard WSGI_ application. actpy
provides the base for a WSGI launcher script as ``actpy-wsgi.example.py``. That
script should be customized (possibly after copying it from the setup directory) to correctly set the
configuration directly in :mod:`actpy.tools.config` rather than through the
command-line or a configuration file.

However the WSGI server will only expose the main HTTP endpoint for the web
client, website and webservice API. Because actpy does not control the creation
of workers anymore it can not setup cron or livechat workers

Cron Workers
------------

To run cron jobs for an actpy deployment as a WSGI application requires

* A classical actpy (run via ``actpy-bin``)
...
* Which should not be exposed to the network. To ensure cron runners are not
  ...
  entirely with :option:`actpy-bin --no-http` or setting ``http_enable = False``
...
* Deploy a threaded version of actpy (instread of a process-based preforking
  one) and redirect only requests to URLs starting with ``/longpolling/`` to
  that actpy, this is the simplest and the longpolling URL can double up as
  the cron instance.
* Deploy an evented actpy via ``actpy-gevent`` and proxy requests starting
  with ``/longpolling/`` to
  :option:`the longpolling port <actpy-bin --longpolling-port>`.

Serving Static Files
====================

For development convenience, actpy directly serves all static files in its
...
actpy static files live in each module's ``static/`` folder, so static files
...
- Use appropriate database filters ( :option:`--db-filter <actpy-bin --db-filter>`)
  to restrict the visibility of your databases according to the hostname.
  See :ref:`db_filter`.
  You may also use :option:`-d <actpy-bin -d>` to provide your own (comma-separated)
...
- Once your ``db_name`` and ``db_filter`` are configured and only match a single database
  ...
  (this is also exposed as the :option:`--no-database-list <actpy-bin --no-database-list>`
  command-line option)

- Make sure the PostgreSQL user (:option:`--db_user <actpy-bin --db_user>`) is *not* a super-user,
  ...
  See also :ref:`setup/deploy/actpy`.
...
- Run actpy behind a web server providing HTTPS termination with a valid SSL certificate,
  ...
  and then enable the :option:`proxy mode <actpy-bin --proxy-mode>` option.
...
:ref:`setup/deploy/actpy` mentioned ``admin_passwd`` in passing.
...
actpy is supported by multiple browsers for each of its versions. No 
distinction is made according to the browser version in order to be
up-to-date. actpy is supported on the current browser version. The list 
of the supported browsers by actpy version is the following:

- **actpy 1.0+:** Mozilla Firefox, Google Chrome, Safari, Microsoft Edge

.. [#different-machines]
    to have multiple actpy installations use the same PostgreSQL database,
...
.. _POSBox: https://www.actpy.com/page/point-of-sale-hardware#part_2
