...
Official compatibility: actpy 1.0 will be the first LTS release to introduce
...
used by the official actpy distribution have been ported and are considered
...
.. important::
    ...
    a guide to write 2/3-compatible actpy code. It does not go through all the
    changes in Python but rather through issues which have been found in the
    standard actpy distribution in order to show how to evolve such code such
...
A cross compatible actpy would only support Python 2.7 and Python 3.5 and
...
.. warning::
    ...
    generator delegation, pathlib, ...), you must *not* use them in actpy
    until Python 2 support is dropped

.. note::

    In the *very rare* cases where you *need* to differentiate between
    Python 2 and Python 3, use the :data:`actpy.tools.pycompat.PY2` flag.
...
* fixing dependencies
* using a ``collections.OrderedDict`` or ``actpy.tools.misc.OrderedSet`` instead
...
* ``urllib``, ``urllib2`` and ``urlparse`` were redistributed across
  ...
  Since `requests`_ and `werkzeug`_ are already hard dependencies of actpy,
...
* ``cgi.escape`` (HTML escaping) is deprecated in Python 3, prefer actpy's own
  :func:`actpy.tools.misc.html_encode`.
...
actually happened in actpy with :mod:`actpy.tools.mimetypes`.
...
.. important::
    ...
    * ``(int, long)`` for type-checking purposes must be replaced by
      :py:data:`actpy.tools.pycompat.integer_types`
...
* type-testing is the last and bigger issue as in Python 2 ``long`` is not a
  ...
  For that case, actpy 11 now provides a compatibility module with an
  :py:data:`~actpy.tools.pycompat.integer_types` definition which can be used
  for type-testing.
...
For easier type-testing, :mod:`actpy.tools.pycompat` provides the following
constants:

* :data:`~actpy.tools.pycompat.string_types` is an alias/type tuple for testing
  string types, essentially a replacement of testing for ``basestring`` or
  ``(str, unicode)``.
* :data:`~actpy.tools.pycompat.text_type` is the proper *text* type for the
...
However with respect to actpy it turns out most or all uses of ``csv`` fit
inside a model of *byte stream to and from text values*.

The latter is thus a model implemented by cross-version wrappers
:func:`actpy.tools.pycompat.csv_reader` and
:func:`actpy.tools.pycompat.csv_writer`: they take a *UTF-8 byte stream* and
