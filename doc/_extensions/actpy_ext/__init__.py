    ...
    if hasattr(app, 'set_translator'):
        ...
        if getattr(app.config, 'html_translator_class', None):
            app.warn("Overriding the explicitly set  html_translator_class setting",
                     location="actpy extension")
        app.config.html_translator_class = 'actpy_ext.translator.BootstrapTranslator'

    switcher.setup(app)
    app.add_config_value('actpy_cover_default', None, 'env')
    app.add_config_value('actpy_cover_external', {}, 'env')
    app.add_config_value('actpy_cover_default_external', lambda conf: conf.actpy_cover_default, 'env')
...
def update_meta(app, pagename, templatename, context, doctree):
    ...
    meta.setdefault('banner', app.config.actpy_cover_default)
