...
# Utils
#----------------------------------------------------------
exec(open(join(dirname(__file__), '..', 'actpy', 'release.py'), 'rb').read())
...
def publish(o, type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/actpy_*.%s" % (o.build_dir, extension))
        if release:
            published.append(_publish(o, release[0]))
    return published

class actpyDocker(object):
    def __init__(self):
...
class KVM(object):
    def __init__(self, o, image, ssh_key='', login='actpy'):
        self.o = o
        self.image = image
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/actpybuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing actpy files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        setuppath = glob("%s/actpy-server-setup-*.exe" % self.o.build_dir)[0]
        setupfile = setuppath.split('/')[-1]
        setupversion = setupfile.split('actpy-server-setup-')[1].split('.exe')[0]

        self.rsync('"%s" %s@127.0.0.1:' % (setuppath, self.login))
        self.ssh("TEMP=/tmp ./%s /S" % setupfile)
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"actpy %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"actpy {sv}"/python/python.exe \'c:\\Program Files\\actpy {sv}\\server\\actpy-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
...
def _prepare_build_dir(o, win32=False):
    ...
    system(cmd + ['%s/' % o.actpy_dir, o.build_dir])
    for addon_path in glob(join(o.build_dir, 'addons/*')):
        if addon_path.split(os.path.sep)[-1] not in ADDONS_NOT_TO_PUBLISH:
            try:
                shutil.move(addon_path, join(o.build_dir, 'actpy/addons'))
            except shutil.Error as e:
                # Thrown when the add-on is already in actpy/addons (if _prepare_build_dir
...
def build_tgz(o):
    ...
    system(['mv', glob('%s/dist/actpy-*.tar.gz' % o.build_dir)[0], '%s/actpy_%s.%s.tar.gz' % (o.build_dir, version, timestamp)])
    system(['mv', glob('%s/dist/actpy-*.zip' % o.build_dir)[0], '%s/actpy_%s.%s.zip' % (o.build_dir, version, timestamp)])

def build_deb(o):
    # Append timestamp to version for the .dsc to refer the right .tar.gz
    cmd=['sed', '-i', '1s/^.*$/actpy (%s.%s) stable; urgency=low/'%(version,timestamp), 'debian/changelog']
    ...
    wildcards = ['actpy_{}'.format(wc) for wc in ('*.deb', '*.dsc', '*_amd64.changes', '*.tar.gz', '*.tar.xz')]
    move_glob(build_dir_parent, wildcards, o.build_dir)

def build_rpm(o):
    system(['python3', 'setup.py', '--quiet', 'bdist_rpm'], o.build_dir)
    system(['mv', glob('%s/dist/actpy-*.noarch.rpm' % o.build_dir)[0], '%s/actpy_%s.%s.noarch.rpm' % (o.build_dir, version, timestamp)])

def build_exe(o):
    KVMWinBuildExe(o, o.vm_winxp_image, o.vm_winxp_ssh_key, o.vm_winxp_login).start()
    system(['cp', glob('%s/actpy*.exe' % o.build_dir)[0], '%s/actpy_%s.%s.exe' % (o.build_dir, version, timestamp)])
...
def _prepare_testing(o):
    ...
    if not o.no_tarball:
        ...
        subprocess.call(["cp", "package.dfsrc", os.path.join(o.build_dir, "docker_src", "Dockerfile")],
                        cwd=os.path.join(o.actpy_dir, "setup"))
        ...
        subprocess.call(["rsync", "-a", "requirements.txt", os.path.join(o.build_dir, "docker_src")],
                        cwd=os.path.join(o.actpy_dir))
        subprocess.call(["docker", "build", "-t", "actpy-%s-src-nightly-tests" % docker_version, "."],
    ...
    if not o.no_debian:
        ...
        subprocess.call(["rsync", "-a", "requirements.txt", os.path.join(o.build_dir, "docker_debian")],
                        cwd=os.path.join(o.actpy_dir))
        subprocess.call(["docker", "build", "-t", "actpy-%s-debian-nightly-tests" % docker_version, "."],
    ...
    if not o.no_rpm:
        ...
        subprocess.call(["cp", "package.dffedora", os.path.join(o.build_dir, "docker_fedora", "Dockerfile")],
                        cwd=os.path.join(o.actpy_dir, "setup"))
        subprocess.call(["docker", "build", "-t", "actpy-%s-fedora-nightly-tests" % docker_version, "."],
                        cwd=os.path.join(o.build_dir, "docker_fedora"))

def test_tgz(o):
    logging.info('Testing tarball in docker')
    with docker('actpy-%s-src-nightly-tests' % docker_version, o.build_dir, o.pub) as wheezy:
        wheezy.release = '*.tar.gz'
        wheezy.system("service postgresql start")
        wheezy.system('pip3 install /opt/release/%s' % wheezy.release)
        wheezy.system("useradd --system --no-create-home actpy")
        wheezy.system('su postgres -s /bin/bash -c "createuser -s actpy"')
        wheezy.system('su postgres -s /bin/bash -c "createdb mycompany"')
        wheezy.system('mkdir /var/lib/actpy')
        wheezy.system('chown actpy:actpy /var/lib/actpy')
        wheezy.system('su actpy -s /bin/bash -c "actpy -d mycompany -i base --stop-after-init"')
        wheezy.system('su actpy -s /bin/bash -c "actpy -d mycompany &"')

def test_deb(o):
    logging.info('Testing deb package in docker')
    with docker('actpy-%s-debian-nightly-tests' % docker_version, o.build_dir, o.pub) as wheezy:
        wheezy.release = '*.deb'
        wheezy.system("service postgresql start")
        wheezy.system('su postgres -s /bin/bash -c "createdb mycompany"')
        wheezy.system('/usr/bin/dpkg -i /opt/release/%s' % wheezy.release)
        wheezy.system('/usr/bin/apt-get install -f -y')
        wheezy.system('su actpy -s /bin/bash -c "actpy -c /etc/actpy/actpy.conf -d mycompany -i base --stop-after-init"')
        wheezy.system('su actpy -s /bin/bash -c "actpy -c /etc/actpy/actpy.conf -d mycompany &"')

def test_rpm(o):
    logging.info('Testing rpm in docker')
    with docker('actpy-%s-fedora-nightly-tests' % docker_version, o.build_dir, o.pub) as fedora24:
        ...
        fedora24.system('su actpy -s /bin/bash -c "actpy -c /etc/actpy/actpy.conf -d mycompany -i base --stop-after-init"')
        fedora24.system('su actpy -s /bin/bash -c "actpy -c /etc/actpy/actpy.conf -d mycompany &"')
...
def options():
    ...
    # Windows VM
    op.add_option("", "--vm-winxp-image", default='/home/actpy/vm/win1036/win10_winpy36.qcow2', help="%default")
    op.add_option("", "--vm-winxp-ssh-key", default='/home/actpy/vm/win1036/id_rsa', help="%default")
    ...
    o.work = join(o.build_dir, 'actpy-%s' % version)
    o.work_addons = join(o.work, 'actpy', 'addons')
