...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\actpy Server ${VERSION}"
...
OutFile "actpy-allinone-setup-${VERSION}.exe"
...
InstallDir "$PROGRAMFILES\actpy ${VERSION}"
...
#VIAddVersionKey "LegalTrademark" "actpy is a trademark of ${PUBLISHER}"
...
!define MUI_ICON "${PIXMAPS_PATH}\actpy-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\actpy-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\actpy-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\actpy-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://actpy.com/page/contactus"
...
; English
LangString DESC_actpy_Server ${LANG_ENGLISH} "Install the actpy Server with all the actpy standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by actpy."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact actpy for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the actpy Server"
...
LangString TITLE_actpy_Server ${LANG_ENGLISH} "actpy Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start actpy"

; French
LangString DESC_actpy_Server ${LANG_FRENCH} "Installation du Serveur actpy avec tous les modules actpy standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par actpy."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez actpy pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur actpy"
...
LangString TITLE_actpy_Server ${LANG_FRENCH} "Serveur actpy"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer actpy"

InstType $(Profile_AllInOne)
InstType $(Profile_Server)

Section $(TITLE_actpy_Server) Sectionactpy_Server
    SectionIn 1 2
...
# If there is a previous install of the actpy Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\actpy.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\actpy.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\actpy.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\actpy.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\actpy.conf" "options" "addons_path" "$INSTDIR\server\actpy\addons,$INSTDIR\server\addons"
    WriteIniStr "$INSTDIR\server\actpy.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\actpy.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\actpy-bin" --stop-after-init --logfile "$INSTDIR\server\actpy.log" -s'
    ${If} ${RunningX64}
      nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe" "\"$INSTDIR\server\actpy-bin\""'
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\${POSTGRESQL_EXE_FILENAME}" \
        ...
        --servicename "PostgreSQL_For_actpy" \
...
;    WriteRegDWORD HKLM     "${UNINSTALL_REGISTRY_KEY}" "VersionMinor" "${REVISION_VERSION}"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@actpy.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+65.00.00.00.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://actpy.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@actpy.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${Sectionactpy_Server} $(DESC_actpy_Server)
...
Function ShowPostgreSQL
    SectionGetFlags ${Sectionactpy_Server} $0
