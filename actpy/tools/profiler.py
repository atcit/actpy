...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if isinstance(in_self, (actpy.sql_db.Cursor, actpy.sql_db.TestCursor, actpy.sql_db.LazyCursor)):
            return self.tracer
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        all methods for all actpy models by applying the optional filters.
        ...
        :param list blacklist: list model names to remove from the log
                        (Default: remove non actpy model from the log: [None])
        ...
        .. code-block:: python
          ...
          from actpy.tools.profiler import profile
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @api.multi
            @profile()                  # log all methods for all actpy models
            def unlink(self):
            ...
            @profile(whitelist=['sale.order', 'ir.model.data'])
            def action_quotation_send(self):
            ...
            @profile(files=['/home/actpy/actpy/actpy/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _actpyProfile(method, *args, **kwargs):
    ...
    if not method:
        return lambda method: decorator(_actpyProfile, method)

    wrapper = decorator(_actpyProfile, method)
    return wrapper
