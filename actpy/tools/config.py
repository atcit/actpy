#actpy.loggers.handlers. -*- coding: utf-8 -*-
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            ...
            'publisher_warranty_url':
                'http://services.actpy.com/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        }
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.actpyrc (or to ~/.actpy_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store actpy data")
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "actpy.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="actpy.http.rpc.request:DEBUG", help='shortcut for --log-handler=actpy.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="actpy.http.rpc.response:DEBUG", help='shortcut for --log-handler=actpy.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="actpy.http:DEBUG", help='shortcut for --log-handler=actpy.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="actpy.sql_db:DEBUG", help='shortcut for --log-handler=actpy.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options. ",
            "Use these options to translate actpy to another language. "
    ...
    def parse_config(self, args=None):
        ...
        This method initializes actpy.tools.config and actpy.conf (the
        ...
        Typical usage of this method:
            ...
            actpy.tools.config.parse_config(sys.argv[1:])
        """
        self._parse_config(args)
        actpy.netsvc.init_logger()
        actpy.modules.module.initialize_sys_path()
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'actpy.conf')
        else:
            rcfilepath = os.path.expanduser('~/.actpyrc')
            old_rcfilepath = os.path.expanduser('~/.actpy_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.actpyrc' and '.actpy_serverrc' in your path. Please keep only one of "\
                "them, preferrably '.actpyrc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('actpy_RC') or os.environ.get('OPENERP_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from actpy.modules.module import MANIFEST_NAMES
