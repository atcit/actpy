...
import actpy
from actpy.loglevels import ustr
from actpy.tools import pycompat, misc
...
# Typical form of references is <timestamp-actpy-record_id-model_name@domain>
# group(1) = the record ID ; group(2) = the model (if any) ; group(3) = the domain
reference_re = re.compile("<.*-actpy-(\\d+)(?:-([\w.]+))?[^>]*@([^>]*)>", re.UNICODE)
discussion_re = re.compile("<.*-actpy-private[^>]*@([^>]*)>", re.UNICODE)
...
def generate_tracking_message_id(res_id):
    ...
    try:
        ...
        rnd = random.random()
    rndstr = ("%.15f" % rnd)[2:]
    return "<%s.%.15f-actpy-%s@%s>" % (rndstr, time.time(), res_id, socket.gethostname())
