...
from actpy.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if actpy.tools.config['db_host']:
        env['PGHOST'] = actpy.tools.config['db_host']
    if actpy.tools.config['db_port']:
        env['PGPORT'] = str(actpy.tools.config['db_port'])
    if actpy.tools.config['db_user']:
        env['PGUSER'] = actpy.tools.config['db_user']
    if actpy.tools.config['db_password']:
        env['PGPASSWORD'] = actpy.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    ...
    """
    import actpy.modules as addons
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = actpy.modules.module.get_resource_path('base', 'res', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('actpy.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('actpy.foo.bar'):
            do_suff()
