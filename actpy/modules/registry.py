...
from actpy.tools import (assertion_report, config, existing_tables,
                        lazy_classproperty, lazy_property, table_exists,
                        topological_sort, OrderedSet)
from actpy.tools.lru import LRU
...
class Registry(Mapping):
    ...
    def __new__(cls, db_name):
        ...
        with cls._lock:
            try:
                ...
                # actpy.service.wsgi_server.application
        ...
        with cls._lock:
            with actpy.api.Environment.manage():
                ...
                try:
                    ...
                    try:
                        actpy.modules.load_modules(registry._db, force_demo, status, update_module)
                    except Exception:
    ...
    def init(self, db_name):
        ...
        self._db = actpy.sql_db.db_connect(db_name)
        ...
        # Inter-process signaling (used only when actpy.multi_process is True):
        ...
        with closing(self.cursor()) as cr:
            has_unaccent = actpy.modules.db.has_unaccent(cr)
            if actpy.tools.config['unaccent'] and not has_unaccent:
                _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
            self.has_unaccent = actpy.tools.config['unaccent'] and has_unaccent
    ...
    def init_models(self, cr, model_names, context):
        ...
        env = actpy.api.Environment(cr, SUPERUSER_ID, context)
    ...
    def signal_changes(self):
        ...
        if actpy.multi_process and self.registry_invalidated:
        ...
        elif actpy.multi_process and self.cache_invalidated:
