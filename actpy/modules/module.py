...
import actpy
import actpy.tools as tools
import actpy.release as release
from actpy import SUPERUSER_ID, api
...
class AddonsHook(object):
    """ Makes modules accessible through openerp.addons.* and actpy.addons.* """

    def find_module(self, name, path=None):
        if name.startswith(('actpy.addons.', 'openerp.addons.'))\
                and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        # get canonical names
        actpy_name = re.sub(r'^openerp.addons.(\w+)$', r'actpy.addons.\g<1>', name)
        openerp_name = re.sub(r'^actpy.addons.(\w+)$', r'openerp.addons.\g<1>', actpy_name)

        assert actpy_name not in sys.modules
        ...
        # create empty actpy.addons.* module, set name
        new_mod = types.ModuleType(actpy_name)
        new_mod.__loader__ = self

        # module top-level can only be a package
        assert type_ == imp.PKG_DIRECTORY, "actpy addon top-level must be a package"
        modfile = opj(path, '__init__.py')
        new_mod.__file__ = modfile
        new_mod.__path__ = [path]
        new_mod.__package__ = actpy_name

        # both base and alias should be in sys.modules to handle recursive and
        # corecursive situations
        sys.modules[actpy_name] = sys.modules[openerp_name] = new_mod
        ...
        setattr(actpy.addons, addon_name, new_mod)
...
class actpyHook(object):
    """ Makes actpy package also available as openerp """
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'actpy\g<1>', name)
...
def initialize_sys_path():
    ...
    ``import actpy.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global ad_paths
    global hooked
    ...
    # add actpy.addons.__path__
    for ad in __import__('actpy.addons').addons.__path__:
        ad = os.path.abspath(ad)
        if ad not in ad_paths and os.path.isdir(ad):
            ad_paths.append(ad)

    if not hooked:
        sys.meta_path.insert(0, actpyHook())
...
def get_module_filetree(module, dir='.'):
    ...
    files = actpy.tools.osutil.listdir(path, True)
...
def load_openerp_module(module_name):
    ...
    initialize_sys_path()
    try:
        __import__('actpy.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['actpy.addons.' + module_name], info['post_load'])()
...
class TestStream(object):
    def __init__(self, logger_name='actpy.tests'):
...
def run_unit_tests(module_name, dbname, position=runs_at_install):
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, actpy.sql_db.sql_counter - t0_sql)
