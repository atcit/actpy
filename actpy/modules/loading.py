...
import actpy
import actpy.modules.db
import actpy.modules.graph
import actpy.modules.migration
import actpy.modules.registry
import actpy.tools as tools

from actpy import api, SUPERUSER_ID
from actpy.modules.module import adapt_version, initialize_sys_path, load_openerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('actpy.tests')
...
def load_module_graph(cr, graph, status=None, perform_checks=True, skip_modules=None, report=None):
        ...
        if hasattr(package, 'init') or hasattr(package, 'update') or package.state in ('to install', 'to upgrade'):
            ...
            if has_demo:
                ...
                if tools.config.options['test_enable']:
                    ...
                    report.record_result(actpy.modules.module.run_unit_tests(module_name, cr.dbname))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, actpy.sql_db.sql_counter - t0_sql)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['actpy.addons.%s' % (pkg.name,)]
