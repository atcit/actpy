...
# *broken*
NO_POSTMORTEM = (actpy.osv.orm.except_orm,
                 actpy.exceptions.AccessError,
                 actpy.exceptions.ValidationError,
                 actpy.exceptions.MissingError,
                 actpy.exceptions.AccessDenied,
                 actpy.exceptions.Warning,
                 actpy.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                actpy.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = actpy.service.common.dispatch
        elif service_name == 'db':
            dispatch = actpy.service.db.dispatch
        elif service_name == 'object':
            dispatch = actpy.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                actpy.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                actpy.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except actpy.exceptions.DeferredException as e:
        _logger.exception(actpy.tools.exception_to_unicode(e))
        actpy.tools.debugger.post_mortem(actpy.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(actpy.tools.exception_to_unicode(e))
        actpy.tools.debugger.post_mortem(actpy.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all actpy Web request types, mostly deals with
    ...
    @property
    def cr(self):
        """ :class:`~actpy.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~actpy.api.Environment` bound to current request. """
        if self._env is None:
            self._env = actpy.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            actpy.tools.debugger.post_mortem(
                actpy.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        actpy implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <actpy.http.WebRequest.csrf_token`, the
          :data:`~actpy.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          actpy) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (actpy.exceptions.Warning, SessionExpiredException,
                                          actpy.exceptions.except_orm, werkzeug.exceptions.NotFound)):
                _logger.exception("Exception during JSON request handling.")
            error = {
                    'code': 200,
                    'message': "actpy Server Error",
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "actpy Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "actpy Session Expired"
...
def serialize_exception(e):
    ...
    if isinstance(e, actpy.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, actpy.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, actpy.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, actpy.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, actpy.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, actpy.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, actpy.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, actpy.exceptions.except_orm):
...
actpy URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.actpy.com/documentation/11.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through actpy via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["actpy", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('actpy.addons.') and c.__module__.split(".")[2] in modules
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or actpy.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = actpy.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        return routing_map([''] + actpy.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in actpy.modules.module.ad_paths:
            for module in sorted(os.listdir(str(addons_path))):
                if module not in addons_module:
                    ...
                    if manifest_path and os.path.isdir(path_static):
                        ...
                        if 'actpy.addons' in sys.modules:
                            m = __import__('actpy.addons.' + module)
                        else:
                            m = None
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        actpy.registry(db).check_signaling()
                        with actpy.tools.mute_logger('actpy.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = actpy.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if actpy.tools.config['dbfilter']:
        r = actpy.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif actpy.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, actpy will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in actpy.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = actpy.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = actpy.tools.ustr(filename)
