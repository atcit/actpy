...
import actpy
from actpy.modules.module import run_unit_tests, runs_post_install
from actpy.modules.registry import Registry
from actpy.release import nt_service_name
from actpy.tools import config
from actpy.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'actpy.service.http.request.%s' % (me.ident,)
...
class FSWatcher(object):
    def __init__(self):
        ...
        for path in actpy.modules.module.ad_paths:
            ...
            if not event.is_directory:
                ...
                if path.endswith('.py'):
                    try:
                        ...
                        if not getattr(actpy, 'phoenix', False):
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from actpy.addons.base.ir.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = actpy.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(actpy.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="actpy.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="actpy.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        _logger.debug('--')
        actpy.modules.registry.Registry.delete_all()
        logging.shutdown()
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('actpy: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(actpy.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = actpy.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('actpy.netsvc.rpc.request')
...
def load_server_wide_modules():
    for m in actpy.conf.server_wide_modules:
        try:
            actpy.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if actpy.tools.osutil.is_running_as_nt_service():
...
def load_test_file_yml(registry, test_file):
    with registry.cursor() as cr:
        actpy.tools.convert_yaml_import(cr, 'base', open(test_file, 'rb'), 'test', {}, 'init')
    ...
    for mod_name, mod_mod in list(sys.modules.items()):
        if mod_mod:
            ...
            if test_path == mod_path:
                ...
                stream = actpy.modules.module.TestStream()
...
def start(preload=None, stop=False):
    """ Start the actpy http server and cron processor.
    """
    global server

    load_server_wide_modules()
    actpy.service.wsgi_server._patch_xmlrpc_marshaller()

    if actpy.evented:
        server = GeventServer(actpy.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(actpy.service.wsgi_server.application)
        ...
        server = ThreadedServer(actpy.service.wsgi_server.application)
    ...
    if getattr(actpy, 'phoenix', False):
        if watcher:
            watcher.stop()
        _reexec()
