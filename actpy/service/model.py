...
import actpy
from actpy.exceptions import UserError, ValidationError, QWebException
from actpy.models import check_method_name
from actpy.tools.translate import translate
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            try:
                cr = actpy.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if actpy.registry(dbname)._init and not actpy.tools.config['test_enable']:
                    raise actpy.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    recs = actpy.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    return actpy.api.call_kw(recs, method, args, kw)
