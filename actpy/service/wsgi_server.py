...
# User code must use the exceptions defined in ``actpy.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, actpy.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, actpy.tools.ustr(e.value))
    ...
    elif isinstance(e, actpy.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = actpy.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, actpy.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(actpy.tools.ustr(e), formatted_info)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(actpy.tools.exception_to_unicode(e), formatted_info)
...
def _patch_xmlrpc_marshaller():
    ...
    # Historically, actpy is sending binary as base64 string.
    ...
    def dump_bytes(marshaller, value, write):
        marshaller.dump_unicode(actpy.tools.ustr(value), write)
...
def wsgi_xmlrpc(environ, start_response):
    ...
    if environ['REQUEST_METHOD'] == 'POST' and environ['PATH_INFO'].startswith('/xmlrpc/'):
        ...
        try:
            result = actpy.http.dispatch_rpc(service, method, params)
...
def application_unproxied(environ, start_response):
    ...
    # actpy.service.web_services.objects_proxy.dispatch().
    ...
    with actpy.api.Environment.manage():
        ...
        for handler in [wsgi_xmlrpc, actpy.http.root]:
