...
import actpy
from actpy import SUPERUSER_ID
from actpy.exceptions import AccessDenied
import actpy.release
import actpy.sql_db
import actpy.tools
from actpy.sql_db import db_connect
from actpy.release import version_info
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not actpy.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and actpy.tools.config.verify_admin_password(passwd):
        return True
    raise actpy.exceptions.AccessDenied()

# This should be moved to actpy.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None):
    try:
        ...
        with closing(db.cursor()) as cr:
            ...
            actpy.tools.config['load_language'] = lang
            cr.commit()

        registry = actpy.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = actpy.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = actpy.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = actpy.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = actpy.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    actpy.sql_db.close_db(db_original_name)
    db = actpy.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = actpy.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = actpy.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = actpy.tools.config.filestore(db_original_name)
    to_fs = actpy.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    actpy.modules.registry.Registry.delete(db_name)
    actpy.sql_db.close_db(db_name)
    ...
    fs = actpy.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'actpy_dump': '1',
        'db_name': cr.dbname,
        'version': actpy.release.version,
        'version_info': actpy.release.version_info,
        'major_version': actpy.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with actpy.tools.osutil.tempdir() as dump_dir:
            filestore = actpy.tools.config.filestore(db_name)
            ...
            actpy.tools.exec_pg_command(*cmd)
            if stream:
                actpy.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                actpy.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = actpy.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with actpy.tools.osutil.tempdir() as dump_dir:
        ...
        if actpy.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = actpy.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = actpy.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
            ...
            if actpy.tools.config['unaccent']:
                try:
                    with cr.savepoint():
...
def exp_rename(old_name, new_name):
    actpy.modules.registry.Registry.delete(old_name)
    actpy.sql_db.close_db(old_name)
    ...
    old_fs = actpy.tools.config.filestore(old_name)
    new_fs = actpy.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    actpy.tools.config.set_admin_password(new_password)
    actpy.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        actpy.tools.config['update']['base'] = True
        actpy.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@actpy.tools.mute_logger('actpy.sql_db')
...
def list_dbs(force=False):
    if not actpy.tools.config['list_db'] and not force:
        raise actpy.exceptions.AccessDenied()

    if not actpy.tools.config['dbfilter'] and actpy.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, actpy will not
        ...
        res = sorted(db.strip() for db in actpy.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            db_user = actpy.tools.config["db_user"]
            if not db_user and os.name == 'posix':
                import pwd
                db_user = pwd.getpwuid(os.getuid())[0]
            if not db_user:
                cr.execute("select usename from pg_user where usesysid=(select datdba from pg_database where datname=%s)", (actpy.tools.config["db_name"],))
                res = cr.fetchone()
            ...
            res = [actpy.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
            res = []
    res.sort()
    return res
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of actpy
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if actpy.tools.table_exists(cr, 'ir_module_module'):
                ...
                incompatible_databases.append(database_name)
        # release connection
        actpy.sql_db.close_db(database_name)
...
def exp_list_lang():
    return actpy.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(actpy.tools.config['root_path'], 'addons/base/res/res_country_data.xml')).getroot()
