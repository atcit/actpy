...
from actpy import api, fields, models, tools, SUPERUSER_ID, _
from actpy.exceptions import UserError, AccessError
from actpy.tools.safe_eval import safe_eval
from actpy.tools.misc import find_in_path
from actpy.tools import config
from actpy.sql_db import TestCursor
...
# here to init the T1 fonts cache at the start-up of actpy so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at actpy start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start actpy with at least two workers to print a pdf version of the reports.')
