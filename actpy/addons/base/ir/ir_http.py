...
import actpy
from actpy import api, http, models, tools, SUPERUSER_ID
from actpy.exceptions import AccessDenied, AccessError
from actpy.http import request, STATIC_CACHE, content_disposition
from actpy.tools import pycompat, consteq
from actpy.tools.mimetypes import guess_mimetype
from actpy.modules.module import get_resource_path, get_module_path
...
class IrHttp(models.AbstractModel):
    ...
    def routing_map(cls):
        if not hasattr(cls, '_routing_map'):
            ...
            if tools.config['test_enable'] and actpy.modules.module.current_test:
                installed.add(actpy.modules.module.current_test)
            mods = [''] + actpy.conf.server_wide_modules + sorted(installed)
