...
__author__ = actpy.release.author
__version__ = actpy.release.version

# Also use the `actpy` logger for the main script.
_logger = logging.getLogger('actpy')
...
def report_configuration():
    ...
    """
    config = actpy.tools.config
    _logger.info("actpy version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', actpy.modules.module.ad_paths)
...
def setup_pid_file():
    ...
    """
    config = actpy.tools.config
    if not actpy.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = actpy.modules.registry.Registry.new(dbname)
        with actpy.api.Environment.manage():
            with registry.cursor() as cr:
                actpy.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with actpy.api.Environment.manage():
        with registry.cursor() as cr:
            actpy.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    actpy.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                actpy.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except actpy.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        actpy.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = actpy.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the actpy server (default command)"""
    def run(self, args):
        main(args)
