actpy.define('mass_mailing.editor', function (require) {
"use strict";
...
var actpy_top = window.top.actpy;
...
snippets_editor.Class.include({
    _computeSnippetTemplates: function (html) {
        ...
        $mail_themes_upgrade.on("click", "> a", function (e) {
            ...
            e.preventDefault();
            actpy_top[window.callback+"_do_action"]("mass_mailing.action_mass_mailing_configuration");
        });
        ...
        $dropdown.on("click", "li > a", function (e) {
            ...
            // Notify form view
            actpy_top[window.callback+"_downup"]($editable_area.addClass("o_dirty").html());
        });
...
actpy_top[callback+"_updown"] = function (value, fields_values, field_name) {
    if (!window || window.closed) {
        delete actpy_top[callback+"_updown"];
        return;
    }
    ...
    if (fields_values.mailing_model && editor_enable) {
        if (value.indexOf('on_change_model_and_list') !== -1) {
            actpy_top[callback+"_downup"](_val);
        }
    }
};
