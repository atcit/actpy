actpy.define('sms.sms_widget_tests', function (require) {
"use strict";
...
QUnit.module('fields', {
    ...
    QUnit.test('Sms widgets are correctly rendered', function (assert) {
        ...
        // GSM-7
        form.$('.o_input').val("Hello from actpy").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 / 1 SMS (GSM7) ', 'Should be "15 / 1 SMS (GSM7) " for "Hello from actpy"');
        // GSM-7 with \n => this one count as 2 characters
        form.$('.o_input').val("Hello from actpy\n").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '17 / 1 SMS (GSM7) ', 'Should be "17 / 1 SMS (GSM7) " for "Hello from actpy\\n"');
        // Unicode => ê
        form.$('.o_input').val("Hêllo from actpy").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 / 1 SMS (UNICODE) ', 'Should be "15 / 1 SMS (UNICODE) " for "Hêllo from actpy"');
        // GSM-7 with 160c
