...
import actpy
import actpy.modules.registry
from actpy.api import call_kw, Environment
from actpy.modules import get_resource_path
from actpy.tools import crop_image, topological_sort, html_escape, pycompat
from actpy.tools.translate import _
from actpy.tools.misc import str2bool, xlwt, file_open
from actpy.tools.safe_eval import safe_eval
from actpy import http
from actpy.http import content_disposition, dispatch_rpc, request, \
    serialize_exception as _serialize_exception, Response
from actpy.exceptions import AccessError, UserError
from actpy.models import check_method_name
from actpy.service import db
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('actpy.addons.web', "views")
    ...
    try:
        ...
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, actpy.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = actpy.conf.server_wide_modules or ['web']
    serverside = []
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("actpy.addons.web.main.manifest_list(): debug parameter is deprecated")
...
def fix_view_modes(action):
    """ For historical reasons, actpy has weird dealings in relation to
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = actpy.SUPERUSER_ID

        values = request.params.copy()
        try:
            values['databases'] = http.db_list()
        except actpy.exceptions.AccessDenied:
...
class WebClient(http.Controller):
    ...
    def version_info(self):
        return actpy.service.common.exp_version()
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = actpy.tools.config.verify_admin_password('admin')
        d['list_db'] = actpy.tools.config['list_db']
        d['langs'] = actpy.service.db.exp_list_lang()
        d['countries'] = actpy.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = actpy.service.db.list_db_incompatible(d['databases'])
        except actpy.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        try:
            actpy.service.db.check_super(master_pwd)
            ...
            dump_stream = actpy.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=actpy.SUPERUSER_ID))
    ...
    def account(self):
        ...
        return 'https://accounts.actpy.com/oauth2/auth?' + werkzeug.url_encode(params)
...
class Binary(http.Controller):
    ...
    def content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        if crop and (width or height):
            ...
            if height > 500:
                height = 500
            content = actpy.tools.image_resize_image(base64_source=content, size=(width or None, height or None), encoding='base64', filetype='PNG')
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = actpy.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: actpy.tools.ustr(field[1].get('string', '')))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from actpy's export data to whatever the
