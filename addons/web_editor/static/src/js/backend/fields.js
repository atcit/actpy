actpy.define('web_editor.backend', function (require) {
'use strict';
...
var FieldTextHtml = AbstractField.extend({
    ...
    start: function () {
        ...
        window.actpy[this.callback+"_editor"] = function (EditorBar) {
            setTimeout(function () {
                self.on_editor_loaded(EditorBar);
            },0);
        };
        window.actpy[this.callback+"_content"] = function () {
            self.on_content_loaded();
        };
        window.actpy[this.callback+"_updown"] = null;
        window.actpy[this.callback+"_downup"] = function () {
            self.resize();
        };
    ...
    on_editor_loaded: function (EditorBar) {
        ...
        if (this.value && window.actpy[self.callback+"_updown"] && !(this.$content.html()||"").length) {
            this.render();
        }
    ...
    render: function () {
        ...
        if (this.mode === "edit") {
            if (window.actpy[this.callback+"_updown"]) {
                // FIXME
                // window.actpy[this.callback+"_updown"](value, this.view.get_fields_values(), this.name);
                this.resize();
            }
        } else {
    ...
    destroy: function () {
        ...
        delete window.actpy[this.callback+"_editor"];
        delete window.actpy[this.callback+"_content"];
        delete window.actpy[this.callback+"_updown"];
        delete window.actpy[this.callback+"_downup"];
    },
