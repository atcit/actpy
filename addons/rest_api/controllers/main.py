    ...
    for field in json_data:
        ...
        if not isinstance(val, list):
            ...
            for res in val:
                ...
                elif isinstance(model_fiels[field], actpy.fields.One2many):
                    if create:
...
db_name = actpy.tools.config.get('db_name')
if not db_name:
    _logger.warning("Warning: To proper setup OAuth - it's necessary to "
                    "set the parameter 'db_name' in actpy config file!")


# List of REST resources in current file:
#   (url prefix)            (method)     (action)
# /api/auth/get_tokens        POST    - Login in actpy and get access tokens
...
class ControllerREST(http.Controller):
    ...
    # Login in actpy database and get access tokens:
    ...
    def api_auth_gettokens(self, **post):
        ...
        # Compare dbname (from HTTP-request vs. actpy config):
        ...
        # actpy login failed:
        if not uid:
            info = "actpy User authentication failed!"
            error = 'actpy_user_authentication_failed'
            _logger.error(info)
