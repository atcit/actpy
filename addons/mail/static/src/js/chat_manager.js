actpy.define('mail.chat_manager', function (require) {
"use strict";
...
function notify_incoming_message (msg, options) {
    if (bus.is_actpy_focused() && options.is_displayed) {
        // no need to notify
        return;
    }
...
function make_message (data) {
    ...
    if ((!msg.author_id || !msg.author_id[0]) && msg.email_from) {
        ...
        msg.displayed_author = (msg.author_id === actpyBOT_ID) && "actpyBot" ||
                               msg.author_id && msg.author_id[1] ||
                               msg.email_from || _t('Anonymous');
    }

    // Don't redirect on author clicked of self-posted or actpyBot messages
    msg.author_redirect = !msg.is_author && msg.author_id !== actpyBOT_ID;

    // Compute the avatar_url
    if (msg.author_id === actpyBOT_ID) {
        msg.avatar_src = "/mail/static/src/img/actpy_o.png";
...
function on_transient_message_notification (data) {
    ...
    data.author_id = data.author_id || actpyBOT_ID;
    add_message(data);
}
