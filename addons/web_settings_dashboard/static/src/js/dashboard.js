actpy.define('web_settings_dashboard', function (require) {
"use strict";
...
var DashboardApps = Widget.extend({
    ...
    start: function() {
        this._super.apply(this, arguments);
        if (actpy.db_info && _.last(actpy.db_info.server_version_info) !== 'e') {
    ...
    confirm_upgrade: function() {
        framework.redirect("https://www.actpy.com/actpy-enterprise/upgrade?num_users=" + (this.data.enterprise_users || 1));
    },
});
...
var DashboardShare = Widget.extend({
    ...
    init: function(parent, data){
        this.data = data;
        this.parent = parent;
        this.share_url = 'https://www.actpy.com';
        this.share_text = encodeURIComponent("I am using #actpy - Awesome open source business apps.");
    },
    ...
    share_linkedin: function(){
        var popup_url = _.str.sprintf('http://www.linkedin.com/shareArticle?mini=true&url=%s&title=I am using actpy&summary=%s&source=www.actpy.com', encodeURIComponent(this.share_url), this.share_text);
        this.sharer(popup_url);
    },
