actpy.define('account.dashboard_setup_bar', function (require) {
"use strict";
...
var AccountSetupBarController = KanbanController.extend({
    ...
    **/
    _triggerCompanyButtonAction: function (actpyEvent) {
        var self = this
        if (actpyEvent.data.rpc_method !== undefined) {
            self._rpc({
                    model: 'res.company',
                    method: actpyEvent.data.rpc_method,
                    args: [],
                })
                .then(
                    function(rslt_action) {
                        if (rslt_action !== undefined) {
                            self.do_action(rslt_action, {
                                action_context: actpyEvent.data.context,
                                on_close: function () {
