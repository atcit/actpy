    ...
    def message_get_email_values(self, notif_mail=None):
        ...
        if res.get('headers'):
            try:
                ...
                pass
        if self.project_id:
            current_objects = [h for h in headers.get('X-actpy-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-actpy-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-actpy-Tags'] = ','.join(self.tag_ids.mapped('name'))
