...
from actpy.exceptions import QWebException
from actpy.tools.safe_eval import safe_eval
from actpy.osv.expression import FALSE_DOMAIN

from actpy.addons.http_routing.models.ir_http import ModelConverter, _guess_mimetype
from actpy.addons.portal.controllers.portal import _build_url_w_params
...
class Http(models.AbstractModel):
    ...
    def _handle_exception(cls, exception):
        ...
        if not is_website_request:
            ...
            if isinstance(exception, actpy.exceptions.AccessError):
                code = 403

            if isinstance(exception, QWebException):
                values.update(qweb_exception=exception)
                if isinstance(exception.qweb.get('cause'), actpy.exceptions.AccessError):
                    code = 403
