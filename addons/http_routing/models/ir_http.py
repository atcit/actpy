...
def is_multilang_url(local_url, langs=None):
    ...
    try:
        ...
        # Force to check method to POST. actpy uses methods : ['POST'] and ['GET', 'POST']
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if actpy._geoip_resolver is not None:
            return
        try:
            import GeoIP
            ...
            if os.path.exists(geofile):
                actpy._geoip_resolver = GeoIP.open(geofile, GeoIP.GEOIP_STANDARD)
            else:
                actpy._geoip_resolver = False
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if actpy._geoip_resolver and request.httprequest.remote_addr:
                record = actpy._geoip_resolver.record_by_addr(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except actpy.exceptions.MissingError:
